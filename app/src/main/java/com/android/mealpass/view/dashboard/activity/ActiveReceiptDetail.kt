...
class ActiveReceiptDetail : DataBindingActivity<ActivityActiveReceiptDetailBinding>(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        requireActiveFoodReceipt(NavigationScreen.EXTRA_ACTIVE_RECEIPT_DETAIL)?.let{
    ...
    private fun requireActiveFoodReceipt(tag: String): ReceiptResponse.Body.ActiveReceipt? {
        var activeReceiptRespose: ReceiptResponse.Body.ActiveReceipt? = null
        intent.getParcelableExtra<ReceiptResponse.Body.ActiveReceipt>(tag)?.let {
